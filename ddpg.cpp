...
namespace ML {
            ...
            int maxMemorySize, int replaceTargetIter, int batchSize)
    {
        ...
        /* actor: a = P(s, theta) */
        this->ActorMainNet.createNetWithSoftmax(stateDim, hiddenDim, hiddenLayerNum, actionDim, ACTIVATE_SIGMOID);
        this->ActorTargetNet.createNetWithSoftmax(stateDim, hiddenDim, hiddenLayerNum, actionDim, ACTIVATE_SIGMOID);
    ...
    void DDPGNet::experienceReplay(Transition& x)
    {
        ...
        /* update ActorMainNet */
        ActorMainNet.calculateBatchGradient(x.state, cMainOutput);
        /* update CriticMainNet */
        CriticMainNet.calculateBatchGradient(x.state, cTarget);
        return;
    }
    ...
    {
        ActorMainNet.saveParameter(actorPara);
        CriticMainNet.saveParameter(criticPara);
        return;
    }

    void DDPGNet::load(const std::string& actorPara, const std::string& criticPara)
    {
        ActorMainNet.loadParameter(actorPara);
        ActorMainNet.softUpdateTo(ActorTargetNet, alpha);
        CriticMainNet.loadParameter(criticPara);
        CriticMainNet.softUpdateTo(CriticTargetNet, alpha);
        return;
    }
}
