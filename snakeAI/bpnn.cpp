...
ML::Layer::Layer(std::size_t inputDim, std::size_t layerDim, LayerType layerType, ActiveType activeType, LossType lossType, bool trainFlag)
{
...
ML::MLP::MLP(std::size_t inputDim, std::size_t hiddenDim, std::size_t hiddenLayerNum, std::size_t outputDim,
...
ML::Vec& ML::MLP::getOutput()
{
    Vec& outputs = layers[outputIndex].O;
    return outputs;
}

void ML::MLP::backPropagate(Vec& yo, Vec& yt)
{
    /*  loss */
...
void ML::MLP::grad(Vec &x, Vec &y, Vec &loss)
{
    /*  loss */
...
void ML::MLP::gradient(Vec &x, Vec &y)
{
    feedForward(x);
...
void ML::MLP::optimize(OptType optType, double learningRate)
{
    switch (optType) {
        case OPT_SGD:
            SGD(learningRate);
