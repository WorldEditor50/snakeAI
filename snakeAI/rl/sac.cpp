...
RL::SAC::SAC(size_t stateDim_, size_t hiddenDim, size_t actionDim_)
{
    ...
    critic1TargetNet = BPNN(Layer<Tanh>::_(criticStateDim, hiddenDim, false),
                            RMSNorm<Sigmoid>::_(hiddenDim, hiddenDim, false),
...
void RL::SAC::experienceReplay(const RL::Transition &x)
{
    ...
    /* train critic net */
    {
        ...
        float nextValue = 0;
        nextValue = nextProb[k]*(q - alpha[k]*std::log(nextProb[k] + 1e-5));
        float qTarget = 0;
        if (x.done) {
            qTarget = x.reward;
        } else {
    ...
    /* train policy net */
    {
        ...
        target[i] = prob[i]*(q - alpha[i]*std::log(prob[i] + 1e-5));
        /* alpha -> 0 */
        alpha.g[i] += -prob[i]*(std::log(prob[i] + 1e-5) + targetEntropy);
