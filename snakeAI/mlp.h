...
public:
    ...
    ~Layer(){}
    explicit Layer(int inputDim, int layerDim, int activateType, int lossTye, int tarinFlag);
...
public:
    MLP(){}
    ~MLP(){}
    explicit MLP(int inputDim, int hiddenDim, int hiddenLayerNum, int outputDim, int trainFlag = 0,
    ...
    void grad(std::vector<double> &x, std::vector<double> &y, std::vector<double> &loss);
